...
## 9. 习题：工具选择失败时如何引导智能体纠正？

不能只把异常抛给模型，也不能无限重试。当前实现采用“校验—反馈—重试—熔断”机制：

1. 执行前校验 JSON、动作类型、工具名和必填参数。
2. 捕获参数错误和执行异常，将失败原因、失败调用、可用工具与纠正要求写入 `Observation`。
3. 要求模型重新核对目标、改选工具或重写参数，不能原样重复失败调用。
4. 成功调用后清零连续失败次数；连续失败达到阈值后终止，避免死循环和无效消耗。

失败反馈必须可执行。只返回“调用失败，请重试”信息不足；返回完整内部异常又可能泄露信息。工程中宜返回稳定错误码和字段级错误。支付、删除等有副作用的工具还应加入幂等键、权限校验和人工确认，不能依赖模型自行重试。

当前代码可处理结构错误、未知工具、工具异常及带错误标记的返回值。更成熟的实现应让工具返回结构化结果，例如 `{"ok": false, "error_code": "INVALID_ARGUMENT", "message": "expression 不能包含等号"}`，避免通过自然语言判断成功或失败。

## 10. 习题：工具增加到 50～100 个时如何组织和检索？

全部工具描述直接塞进每轮提示词，在工具少时直观有效；达到 50～100 个后，会出现上下文、费用和延迟增长，相似工具互相干扰、选错率上升，以及描述难以治理等问题。此时不应继续使用“扁平清单 + 全量注入”，而应使用分层路由：
...
用户请求
  ...
  → 从工具目录召回 Top-K 候选
  → 权限、环境和风险过滤
  → 把少量候选工具及参数 Schema 提供给 ReAct
  → 执行、观察和纠错；必要时扩大召回范围
```

工程优化重点包括：

- **统一元数据**：记录用途、适用/不适用场景、JSON Schema、示例、标签、版本、权限、成本、超时和副作用等级。
- **分类与命名空间**：使用 `math.calculate`、`web.search`、`order.query` 等命名；先选领域，再选具体工具。
- **动态检索**：根据“用户问题 + 当前步骤”做关键词、BM25 或向量检索，只注入相关 Top-K；高频明确意图可用规则直达。
- **权限过滤**：按用户、租户、环境和风险策略预先过滤，模型不应看到无权调用的工具。
- **结构化调用**：使用 JSON Schema、Function Calling 或结构化输出，并在执行前校验类型、范围和必填字段。
- **可观测与评测**：记录候选集、最终选择、参数错误、延迟和成功率，持续评估 Top-K 召回率、选对率和端到端成功率。
- **生命周期治理**：管理工具版本、弃用和兼容关系，避免同义或旧版工具长期共存。

工具检索器也可能选错，所以要保留回退策略：候选集均不适用时扩大 Top-K 或切换领域；多次失败时把原因反馈给检索层，而不是让 ReAct 在同一组工具中循环。
